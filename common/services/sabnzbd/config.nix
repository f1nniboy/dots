...
''
  ...
  url_base = ""
  host_whitelist = ${
    if config.custom.services.caddy.enable then custom.mkServiceDomain config "sabnzbd" else "127.0.0.1"
  },
  local_ranges = ,
  max_url_retries = 10
  ...
  password = "${mkSecret "server/password"}"
  connections = 8
  ssl = 1
  ssl_verify = 2
  ssl_ciphers = ""
